...
const PushModule = (() => {
  ...
  async function registrarSW() {
    ...
    try {
      /* dashboard.html já registra o mesmo sw.js (+ seu próprio listener de
         auto-update) logo antes de disparar "nupi:booted". Registrar de novo
         aqui não cria um segundo SW, mas duplicava o listener de
         controllerchange — toast e reload disparando duas vezes em qualquer
         dispositivo. `ready` só espera o registro que já existe. */
      const reg = await navigator.serviceWorker.ready;
      console.log('[SW] Pronto:', reg.scope);
      ...
      console.error('[SW] Erro ao aguardar registro:', e);
      return null;
    }
  }
